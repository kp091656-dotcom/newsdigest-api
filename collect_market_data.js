...
async function collectChips() {
  ...
  try {
    ...
    // TMF 全體未平倉量（OpenInterestInformation endpoint）
    try {
      const oiUrl = `https://openapi.taifex.com.tw/v1/OpenInterestInformation?queryDate=${dateStr}`;
      const oiRes = await fetch(oiUrl, { headers: { 'User-Agent': 'Mozilla/5.0' }, signal: AbortSignal.timeout(15_000) });
      if (!oiRes.ok) throw new Error(`HTTP ${oiRes.status}`);
      const oiData = await oiRes.json();
      if (!Array.isArray(oiData)) throw new Error('非陣列回傳');
      // Debug：第一次跑印出欄位和商品代碼
      if (oiData.length) console.log(`  🔍 OI 欄位：${Object.keys(oiData[0]).join(', ')}`);
      const allOICodes = [...new Set(oiData.map(r => r.ContractCode || r['商品代號'] || ''))];
      console.log(`  📊 OI 商品代碼（前8）：${allOICodes.slice(0,8).join(', ')}`);
      // 找微型臺指期貨（加總所有月份）
      const tmfOIRows = oiData.filter(r => {
        const code = (r.ContractCode || r['商品代號'] || '').trim();
        return code === '微型臺指期貨' || code === 'TMF';
      });
      if (tmfOIRows.length) {
        const totalOI = tmfOIRows.reduce((sum, r) => {
          // 嘗試各種欄位名稱
          const oi = parseInt(
            (r['OpenInterest'] || r['未沖銷契約量'] || r['OI'] || '0')
            .toString().replace(/,/g, '')
          ) || 0;
          return sum + oi;
        }, 0);
        result.fut_tmf_total_oi = totalOI || null;
        console.log(`  ✅ TMF 全體未平倉：${totalOI} 口（${tmfOIRows.length} 個月份合約）`);
        if (tmfOIRows.length) console.log(`  🔍 TMF OI 樣本：${JSON.stringify(tmfOIRows[0])}`);
      } else {
        console.warn(`  ⚠️  OpenInterestInformation 找不到 TMF，商品代碼：${allOICodes.join(',')}`);
      }
